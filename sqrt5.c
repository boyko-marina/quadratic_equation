...
int test_solve_equation(double a, double b, double c, double *x1, double *x2)
{
    double i, j;
    double a1, b1, c1;
    double x3, x4;
    for (i = -5; i <= 5; i += 0.01)
    {
        for (j = -5; j <= 5; j += 0.01)
        {
            ...
            b1 = -(i+j)
            c1 = i*j
            int n_roots = solve_equation(a1, b1, c1, &x3, &x4);
            if (!(n_roots == 2 && ((x3 == i && x4 == j) || ((x3 == i && x4 == j)))))
            {
                printf("FAILED: solve_equation(%lg, %lg, %lg) -> 2, x=%lg, x=%lg.\n (should be: x=%lg, x=%lg)\n", a1, b1, c1, x3, x4, i, j);
            }
        }
    }
    return 0;
